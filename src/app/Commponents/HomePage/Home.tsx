...
export default function HomePage() {
    ...
    const filterAnimalsByCategory = (category: string) => {
        return category === 'All' ? animals : animals.filter(animal => animal.category === category);
    };

    const filterAnimalsBySearchTerm = (animalsList: Animal[], term: string) => {
        return term
            ? animalsList.filter(animal => animal.category.toLowerCase().includes(term.toLowerCase()))
            : animalsList;
    };

    const displayedAnimals = filterAnimalsBySearchTerm(filterAnimalsByCategory(activeCategory), searchTerm);

    return (
        <>
            <div className="container mx-auto p-4 ">
                <div className="flex flex-wrap gap-4 mb-6">
                    {['All', 'Land Animal', 'Bird', 'Fish', 'Insect', 'Other'].map(category => (
                        <button
                            key={category}
                            onClick={() => handleCategorySelection(category)}
                            className={`px-4 py-2 rounded flex items-center justify-center transition duration-300 ease-in-out ${
                                activeCategory === category ? 'bg-green-500 text-white' : 'bg-gray-200'
                            }`}
                        >
                            {category}
                        </button>
                    ...
                    <div className="flex-grow flex justify-end items-center px-4 space-x-4">
                        <button onClick={handleAnimalModalToggle} className="px-4 py-2 bg-blue-500 text-white rounded">
                            Add Animal
                        </button>
                        <button
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                    {displayedAnimals.map(animal => (
                        <div key={animal.name} className="bg-gray-100 p-4 rounded text-center">
                            <img src={animal.image} alt={animal.name} className="mx-auto mb-2 h-40 w-40 object-cover" />
                            <p className="font-semibold bg-green-400 text-white rounded p-1">
            ...
            <AnimalModal isVisible={isAnimalModalVisible} onClose={handleAnimalModalToggle} />
            <CategoryModal
